...
namespace NetLab_6
{
    public partial class FormMain : Form
    {
        ...
        private void _facultatives_StudyPlanRemoved(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < listViewStudyPlans.Items.Count; i++)
            {
                if ((StudyPlan)listViewStudyPlans.Items[i].Tag == studyPlan)
                {
                    listViewStudyPlans.Items.RemoveAt(i);
                    break;
                }
            }
        }
        ...
        private void editStudyPlanToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (_formStudyPlan.ShowDialog() == DialogResult.OK)
                {
                    studyPlan = _formStudyPlan.StudyPlan;
                    var listViewItem = listViewStudyPlans.SelectedItems[0];
                    listViewItem.Text = studyPlan.Student.ToString();
                    listViewItem.SubItems[1].Text = studyPlan.Subject.ToString();
                }
            }
            catch (Exception)
            {
